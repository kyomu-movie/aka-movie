...
- To share a finished video on GitHub, run `npm.cmd run codex:share -- <project-id>`, then `npm.cmd run codex:push-video -- <project-id>`. This copies only the MP4 and a review note into `shared-videos/`; Git LFS manages the MP4 and the push command rejects other paths.
